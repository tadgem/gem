...
bool rayBoxIntersect(Vec3 rpos, Vec3 rdir, Vec3 vmin, Vec3 vmax, float& t0, float& t1)
{
	float t[10];
...
bool  Im3d::Intersect(const Ray& _ray, const AABB& _aabb, float& t0_, float& t1_)
{
	return rayBoxIntersect(_ray.m_origin, _ray.m_direction, _aabb.m_min, _aabb.m_max, t0_, t1_);
}

bool  Im3d::Contains(const Vec3& _point, const AABB& _aabb)
{
	bool greater = _point.x >= _aabb.m_min.x && _point.y >= _aabb.m_min.y && _point.z >= _aabb.m_min.z;
	bool lesser = _point.x <= _aabb.m_max.x && _point.y <= _aabb.m_max.y && _point.z <= _aabb.m_max.z;
