...
template <typename _Ty, asset_type _AssetType> class asset_t : public asset {
public:
  _Ty m_data;
  asset_t(_Ty data, const std::string &path)
  ...
  void *operator new(size_t size) {
    std::string type_name =
        hash_utils::get_type_name<asset_t<_Ty, _AssetType>>();
    if (debug_memory_tracker::s_instance->s_allocation_info.find(type_name) ==
        debug_memory_tracker::s_instance->s_allocation_info.end()) {
      debug_memory_tracker::s_instance->s_allocation_info.emplace(
          type_name, alloc_info{0, 0});
    }
    debug_memory_tracker::s_instance->s_allocation_info[type_name].count++;
    debug_memory_tracker::s_instance->s_allocation_info[type_name].size += size;
    return malloc(size);
  };
  ...
  void operator delete(void *p) {
    ...
    if (!debug_memory_tracker::s_instance)
      return;
    debug_memory_tracker::s_instance->s_allocation_info[type_name].count--;
    debug_memory_tracker::s_instance->s_allocation_info[type_name].size -=
        sizeof(asset_t<_Ty, _AssetType>);
  };
};
...
template <typename _AssetType, typename _IntermediateType,
          asset_type _AssetTypeEnum>
class asset_t_intermediate : public asset_intermediate {
public:
  _IntermediateType m_intermediate;
  std::string m_path;

  asset_t_intermediate(asset *data, const _IntermediateType &inter,
                       const std::string &path)
      : asset_intermediate(data), m_intermediate(inter), m_path(path) {}

  ~asset_t_intermediate() { m_intermediate.~_IntermediateType(); }
  ...
  void *operator new(size_t size) {
    std::string type_name = hash_utils::get_type_name<
        asset_t_intermediate<_AssetType, _IntermediateType, _AssetTypeEnum>>();
    if (debug_memory_tracker::s_instance->s_allocation_info.find(type_name) ==
        debug_memory_tracker::s_instance->s_allocation_info.end()) {
      debug_memory_tracker::s_instance->s_allocation_info.emplace(
          type_name, alloc_info{0, 0});
  ...
  void operator delete(void *p) {
    ...
    if (!debug_memory_tracker::s_instance)
      return;
    debug_memory_tracker::s_instance->s_allocation_info[type_name].count--;
    debug_memory_tracker::s_instance->s_allocation_info[type_name]
        .size -= sizeof(
        asset_t_intermediate<_AssetType, _IntermediateType, _AssetTypeEnum>);
  };
};
...
template <> struct std::hash<gem::asset_handle> {
  std::size_t operator()(const gem::asset_handle &ah) const {
