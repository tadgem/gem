...
void texture::submit_to_gpu() {
    if(m_mode == mode::stb)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, format, m_width, m_height, 0, format, GL_UNSIGNED_BYTE, m_cpu_data.stb_data);
        glGenerateMipmap(GL_TEXTURE_2D);
    ...
    if(m_mode == mode::gli)
    {
        ...
        gli::gl::format const format = GL.translate(m_cpu_data.gli_data->format(), m_cpu_data.gli_data->swizzles());
        GLenum target = GL.translate(m_cpu_data.gli_data->target());

        glGenTextures(1, &m_handle);
        glBindTexture(target, m_handle);
        glTexParameteri(target, GL_TEXTURE_BASE_LEVEL, 0);
        glTexParameteri(target, GL_TEXTURE_MAX_LEVEL, static_cast<GLint>(m_cpu_data.gli_data->levels() - 1));
        glTexParameteriv(target, GL_TEXTURE_SWIZZLE_RGBA, &format.Swizzles[0]);
        glTexParameteri(target, GL_TEXTURE_WRAP_S, GL_REPEAT);
        glTexParameteri(target, GL_TEXTURE_WRAP_T, GL_REPEAT);
        glTexParameteri(target, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
        glTexParameteri(target, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexStorage2D(target, static_cast<GLint>(m_cpu_data.gli_data->levels()), format.Internal, m_cpu_data.gli_data->extent().x, m_cpu_data.gli_data->extent().y);
        for (std::size_t Level = 0; Level < m_cpu_data.gli_data->levels(); ++Level)
        {
                ...
                glCompressedTexSubImage2D(
                    ...
                    format.Internal, static_cast<GLsizei>(m_cpu_data.gli_data->size(Level)), m_cpu_data.gli_data->data(0, 0, Level));
        }
